...
class MHCState:
    ...
    ``hc_post``). ``hc_fused_post_pre`` is optional so models can replace the
    attention-post/FFN-pre boundary without changing the communicator's
    collective schedule. This class only orchestrates the cross-stage scratch
    (``h_res``, ``h_post``) and exposes the layer's ``hc_mult`` for the few
    sites that need it as a raw scalar.
    """
    ...
    hc_ffn_pre: Callable
    hc_post: Callable
    hc_fused_post_pre: Optional[Callable] = None
    ...
    def attn_to_mlp(
        ...
        out_norm_weight, out_norm_eps = self._resolve_out_norm(out_norm)
        if self.hc_fused_post_pre is None:
    ...
    def __init__(
        ...
        *,
        ...
        self.mhc = MHCState(
            ...
            hc_post=hc_post,
            hc_fused_post_pre=hc_fused_post_pre,
        )
