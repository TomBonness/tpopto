...
class DFlashWorkerV2(BaseSpecWorker):
    ...
    def __init__(
        ...
        if self._use_ngram_primary:
            ...
            self._ngram_history = torch.empty(
                (num_req_slots, self._ngram_window),
                dtype=torch.int64,
                device=self.device,
            )
    ...
    def _seed_ngram_history(self, batch: ScheduleBatch) -> None:
        ...
        for req in batch.reqs:
            ...
            start = max(0, seq_len - self._ngram_window)
            if start == seq_len:
                continue
            tokens = torch.as_tensor(
                fill_ids[start:seq_len], dtype=torch.int64, device=self.device
            )
            slots = torch.arange(
    ...
    def clear_cache_pool(self):
        # Request slots are reseeded from full request tokens on prefill. Clearing
        # prevents stale values from obscuring lifecycle bugs without affecting
    ...
    def forward_batch_generation(
        ...
        if batch.forward_mode.is_extend() or batch.is_extend_in_batch:
            ...
            self._seed_ngram_history(batch)
        ...
        # --- 1) Prepare a fixed verify block, then prefer an exact-history
        # continuation over the neural draft when every greedy request hits.
        ...
        used_ngram = self._try_ngram_primary(
            batch=batch,
            draft_input=draft_input,
            prefix_lens=prefix_lens,
        ...
        if not used_ngram:
            ...
            if lm_head is None or not (
                ...
                or callable(
                    getattr(getattr(lm_head, "quant_method", None), "apply", None)
                )
            ):
                raise RuntimeError(
