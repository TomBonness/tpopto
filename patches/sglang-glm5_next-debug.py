...
class Glm5NextLinearAttention(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # ModelOpt keeps the KDA projections in BF16.  Preserve the fused
        # two-launch path when both fused projection names are explicitly
        # excluded instead of disabling it merely because the model has a
        # global quantization config.
        fused_projection_prefixes = (
            f"{prefix}.fused_qkvbfg_a_proj",
            f"{prefix}.fused_fg_b_proj",
        )
        fused_projections_are_unquantized = quant_config is None or (
            hasattr(quant_config, "is_layer_excluded")
            and all(
                quant_config.is_layer_excluded(projection_prefix)
                for projection_prefix in fused_projection_prefixes
            )
        )
        self.do_fuse_qkvbfg = (
            fused_projections_are_unquantized and head_shard_size == self.tp_size
        )
        ...
        for name, loaded_weight in weights:
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                try:
                    ...
                    print(
                        "GLM53_WEIGHT_LOAD_FAILURE",
                        {
                            "checkpoint_name": original_name,
                            "mapped_name": name,
                ...
                for mapping in expert_params_mapping:
                    ...
                    try:
                        ...
                        print(
                            "GLM53_DIRECT_WEIGHT_LOAD_FAILURE",
                            {
                                "checkpoint_name": original_name,
                                "mapped_name": name,
                                "param_shape": tuple(param.shape),
